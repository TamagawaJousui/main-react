...
export const generateTransformationMatrices = (
    backgroundMatrix,
    noiseMatrix,
    positionMatrix,
    deltaTime
    ...
    const validateMatrix = (matrix) => {
        if (!matrix.rotationAngles || !matrix.scaleFactors || !matrix.translationValues) {
            throw new Error(
                "Matrix must have rotationAngles, scaleFactors, and translationValues properties"
            );
        }
    };
...
const convertToPerFrame = (matrix, deltaTime) => {
...
const generateTransformationMatrix = (matrix) => {
    const { rotationAngles, scaleFactors, translationValues } = matrix;
...
// const getRandomInRange = (a, b) => Math.random() * (b - a) + a;
